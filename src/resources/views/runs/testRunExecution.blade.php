...
@section('sidemenu')
    <div id="sidemenu">
        <button type="button" data-toggle="modal" data-target="#myModal" class="btn btn-default btn-xs {{ $testRun->Status == App\Enums\TestRunStatus::RUNNING ? '' : 'disabled'}}">Finish test run </button>
        <hr />
        <button type="button" class="btn btn-default btn-xs disabled">Not tested yet <span class="badge">{{ $testRun->testCases()->wherePivot('Status', App\Enums\TestCaseStatus::NOT_TESTED)->count() }}</span></button></br></br>
        ...
        <button type="button" class="btn btn-info btn-xs disabled">Blocked <span class="badge">{{ $testRun->testCases()->wherePivot('Status', App\Enums\TestCaseStatus::BLOCKED)->count() }}</span></button>
        <br />
        <hr />
                        ...
                        @if (isset($testCases))
                            @foreach ($testCases as $testCase)
                                @if ($testCase->testSuite->TestSuite_id == $testSuite->TestSuite_id )
                                    <li>
                                        <a href="{{ url("sets_runs/run/execution/$testRun->TestRun_id/testcase/$testCase->TestCase_id") }}" style="padding: 0px 0px 0px 0px; {{ $testCase->TestCase_id == $selectedTestCase->TestCase_id ? 'color:white' : '' }}">
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Do you want close (finish) test run? There is no way back.</p>
                </div>
